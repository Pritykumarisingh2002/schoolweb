...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $upload_path = "uploads/mandate_doc" . $new_name;

    // Delete old file only for SAME category + SAME class
    $old = $pdo->prepare("SELECT * FROM mandate_doc WHERE category = ?");
    // $old->execute([$category, $class_name]);
    $oldFile = $old->fetch(PDO::FETCH_ASSOC);

    if ($oldFile) {
        if (file_exists("uploads/mandate_doc" . $oldFile['file_path'])) {
            unlink("uploads/mandate_doc" . $oldFile['file_path']);
        }

        $pdo->prepare("DELETE FROM mandate_doc WHERE id = ?")
            ->execute([$oldFile['id']]);
    }
    ...
    // Insert new record
    $stmt = $pdo->prepare("INSERT INTO mandate_doc (file_name, file_path, category) VALUES (?, ?, ?)");
